  ...
  <div class="container">
    ...
    <!-- イベントを表示 -->
         <span v-for="event in events">
            <el-tag  v-if=" event.share && $moment(date).format('YYYY-MM-DD') >= $moment(event.startDateTime).format('YYYY-MM-DD') && $moment(date).format('YYYY-MM-DD') <= $moment(event.endDateTime).format('YYYY-MM-DD') " v-bind:type="event.color" size="small"> {{ event.title }} </el-tag>
         </span>
      </div>
    </el-calendar>
